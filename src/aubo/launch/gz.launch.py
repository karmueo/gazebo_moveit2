...
def generate_launch_description() -> LaunchDescription:
    ...
    # xacro2sdf
    # 执行 xacro2sdf.bash 脚本，将 URDF 转换为 SDF
    # 实际命令类似【ros2 run aubo_description xacro2sdf.bash name:=aubo prefix:=aubo_ collision_arm:=true collision_gripper:=false ros2_control:=true ros2_control_plugin:=gz ros2_control_command_interface:=position gazebo_preserve_fixed_joint:=false】
                ...
                on_exit=[
                    ...
                    IncludeLaunchDescription(
                        PythonLaunchDescriptionSource(
                            PathJoinSubstitution(
                                [
                                    FindPackageShare("aubo_moveit_config"),
                                    "launch",
...
def generate_declared_arguments() -> List[DeclareLaunchArgument]:
    ...
    return [
        ...
        DeclareLaunchArgument(
            ...
            description="Custom package with robot description.",  # 机器人描述的自定义包名
        ),
        DeclareLaunchArgument(
            "sdf_model_filepath",
            default_value=path.join("aubo", "model.sdf"),
